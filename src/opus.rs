...
use crate::{format_duration, OggMetadataError};
...
impl AudioMetadata for Metadata {
    ...
    fn get_duration(&self) -> Option<Duration> {
        self.length_in_48khz_samples
            .map(|l| Duration::from_millis(((l as f64) / 48_000. * 1000.0) as u64))
    }
}
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self.length_in_48khz_samples {
            Some(l) => {
                ...
                write!(
                    f,
                    "{} channels, with duration of {}",
                    self.output_channels,
                    format_duration(duration_raw_secs)
                )
            }
