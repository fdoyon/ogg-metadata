...
pub use crate::opus::Metadata as OpusMetadata;
pub use crate::theora::Metadata as TheoraMetadata;
pub use crate::vorbis::Metadata as VorbisMetadata;
use std::error::Error;
...
impl std::error::Error for OggMetadataError {
    ...
    fn cause(&self) -> Option<&dyn std::error::Error> {
        match self {
            OggMetadataError::ReadError(ref err) => Some(err),
            _ => None,
        }
    }
}
...
fn seek_before_end<'a, T: io::Read + io::Seek + 'a>(
    pck_rdr: &mut PacketReader<T>,
    offs: u64,
...
fn get_absgp_of_last_packet<'a, T: io::Read + io::Seek + 'a>(
    pck_rdr: &mut PacketReader<T>,
) -> Result<u64, OggMetadataError> {
...
fn parse_format(
    pck_data: &[u8],
    bare_format: BareOggFormat,
    last_packet_absgp: Option<u64>,
    ...
    Ok(match bare_format {
        ...
        BareOggFormat::Opus => {
            ...
            Opus(OpusMetadata {
                ...
                length_in_48khz_samples: last_packet_absgp.map(|l| l - (ident_hdr.pre_skip as u64)),
            })
        }
...
#[allow(clippy::never_loop)]pub fn read_format<'a, T: io::Read + io::Seek + 'a>(
    rdr: T,
    ...
    if id_inner.1 == BareOggFormat::Skeleton {
        ...
        for (_, stream) in streams.iter() {
            let st = parse_format(&(stream.1).data[(stream.0).0..], (stream.0).1, None)?;
            res.push(st);
        }
    }
